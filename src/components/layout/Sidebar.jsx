...
import { Home, Activity, BarChart3, Bell, Download, User, Shield, Settings, Apple } from 'lucide-react';
...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-gray-800 shadow-lg min-h-[calc(100vh-64px)] sticky top-16 z-30">
            ...
            >
              ...
              <div className="relative flex items-center space-x-3 px-4 py-3">
                <div className={`p-2 rounded-lg ${
                  isActive ? 'bg-white/20' : 'bg-white/50 dark:bg-gray-800/50 group-hover:bg-white/20'
                }`}>
                  <Icon className="w-5 h-5" />
                </div>
                
                <div className="flex-1 text-left">
                  <span className={`font-semibold block transition-all ${isActive ? 'text-white' : 'group-hover:text-white'}`}>
                    {item.name}
                  </span>
                  <span className={`text-xs transition-all ${isActive ? 'text-white/80' : 'text-gray-500 dark:text-gray-400 group-hover:text-white/80'}`}>
                    {item.description}
                  </span>
                </div>
                
                {isActive && <div className="w-1 h-8 bg-white rounded-full shadow-lg"></div>}
              </div>
            </button>
          );
        })} 
